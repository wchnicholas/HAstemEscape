...
WTresibox  <- read_tsv('doc/WSN_WTheatmap.tsv')
SEspTable  <- read_tsv('result/Fitness_WSN.tsv') %>%
...
EspF5_table <- SEspTable %>%
                 mutate(X=esp_FI6v3_200ng)
EspA5_table <- SEspTable %>%
                 mutate(X=esp_CR9114_100ng)
plot_fit_heatmap(EspF5_table,'',WTresibox,'graph/heatmap_FI6v3esp_single_WSN.png')
plot_fit_heatmap(EspA5_table,'',WTresibox,'graph/heatmap_CR9114esp_single_WSN.png')
